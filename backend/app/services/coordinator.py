        ...
        *,
        ...
        for _ in range(_MAX_TOOL_ITERATIONS):
            ...
            if not tool_call_blocks:
                tool_call_blocks = _extract_text_tool_calls(
                    assistant_message,
                    self._tool_registry,
                )
                if tool_call_blocks:
            ...
            for tc_block in tool_call_blocks:
                fallback_key = json.dumps(
                    [tc_block.tool_name, tc_block.arguments],
                    sort_keys=True,
                )
                if fallback_key in executed_fallbacks:
                    yield TextDeltaEvent(delta=executed_fallbacks[fallback_key])
                    return
                ...
                if tc_block.tool_call_id.startswith("fallback-"):
                    executed_fallbacks[fallback_key] = result.output or result.error or ""
...
def _tool_definitions(registry: ToolRegistry | None) -> list[dict[str, object]]:
    ...
    for schema in registry.schemas():
        ...
        required = [parameter.name for parameter in schema.parameters if parameter.required]
        definitions.append({
            "type": "function",
            "function": {
                "name": schema.name,
...
def _extract_text_tool_calls(
    ...
    """Recognize conservative JSON tool-call fallbacks from local models."""
    if registry is None:
        return []
    ...
    for candidate in candidates:
        ...
        if (
            isinstance(name, str)
            and registry.exists(name)
            and isinstance(arguments, dict)
        ):
            return [ToolCallBlock(
                tool_call_id=f"fallback-{uuid4()}",
                tool_name=name,
                arguments=arguments,
            )]
    return []
