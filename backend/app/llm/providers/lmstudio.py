...
class LMStudioProvider(LLMProvider):
    ...
    async def generate(
        ...
        try:
            ...
            if exc.response.status_code == 401:
                raise ProviderAuthenticationError("LM Studio authentication failed") from exc
    ...
    async def generate_stream(
        ...
        try:
            ...
            raise ProviderTimeoutError(
                f"LM Studio stream timed out after {self._timeout}s",
            ) from exc
        except httpx.ConnectError as exc:
            raise ProviderConnectionError(
                f"Could not connect to LM Studio at {self._base_url}",
            ...
            yield StreamErrorEvent(
                ...
                message=f"LM Studio returned {exc.response.status_code}",
            )
