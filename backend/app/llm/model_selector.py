...
class ModelSelector:
    ...
    def _score_providers(
        ...
        for provider in self._router.list_providers():
            ...
            if self._router.is_provider_available(pid):
                mp = self._profile_for_model(mid)
                score = self._score(
    ...
    def _match_profile(
        ...
        """Resolve built-in capabilities for exact or publisher-prefixed IDs."""
    ...
    def _profile_for_model(self, model_id: str) -> ModelProfile | None:
        """Resolve a model profile using exact and family-name matching."""
        return self._match_profile(model_id, self._model_profiles)
