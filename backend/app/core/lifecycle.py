...
async def _discover_lm_studio_models(settings: Settings) -> list[str]:
    """Return model IDs currently exposed by the LM Studio server."""
    ...
    try:
        ...
        logger.info("startup.lm_studio_unavailable", url=url, error=str(exc))
        return []
...
def _lm_studio_provider_id(model_id: str, index: int) -> str:
    """Build a stable provider ID for one LM Studio model."""
    if index == 0:
        return "lm-studio"
    slug = re.sub(r"[^a-z0-9]+", "-", model_id.lower()).strip("-")
    return f"lm-studio-{slug[:70]}"


async def _seed_lm_studio_providers(
    settings: Settings,
    repository: Any,
) -> None:
    """Discover LM Studio models and persist missing provider records."""
    if not settings.lm_studio_auto_discover:
        return

    models = await _discover_lm_studio_models(settings)
    if not models:
        return
    ...
    existing = await repository.find_by_type(ProviderType.LM_STUDIO)
    existing_ids = {str(provider.id) for provider in existing}
    for index, model_id in enumerate(dict.fromkeys(models)):
        provider_id = _lm_studio_provider_id(model_id, index)
        if provider_id in existing_ids:
            continue
        is_vision = "llava" in model_id.lower()
        ...
        await repository.add(
            ProviderSpec(
                ...
                display_name=f"LM Studio: {model_id}",
        ...
        logger.info(
            "startup.lm_studio_provider_registered",
    ...
    * ``app.state.db_engine``
    ...
    try:
        ...
        async with session_context(session_factory) as db_session:
            ...
            await _seed_lm_studio_providers(settings, provider_repo)
